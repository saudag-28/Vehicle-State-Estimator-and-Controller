...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    I = np.identity(3)
    R = I * sensor_var  #it is the measurement noise variance
    # 3.1 Compute Kalman Gain
    K_k = p_cov_check.dot(h_jac.T).dot(np.linalg.inv(h_jac.dot(p_cov_check).dot(h_jac.T) + R))

    # 3.2 Compute error state
    error_delta = K_k.dot(y_k - p_check)

    # 3.3 Correct predicted state
    p_delta = error_delta[:3]
    v_delta = error_delta[3:6]
    q_delta = error_delta[6:]

    p_hat = p_check + p_delta
    v_hat = v_check + v_delta
    q_hat = Quaternion(euler=q_delta).quat_mult_right(q_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    F = np.identity(9)
    F[:3, 3:6] = delta_t * np.identity(3)  #three rows, 4rd to 6th column
    F[3:6, 6:] = -(rotation_matrix.dot(skew_symmetric(imu_f.data[k-1].reshape((3,1)))))  #4th to 6th row, 7th column to end
